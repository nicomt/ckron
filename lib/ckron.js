...
class Ckron {
  ...
  async loadConfig(configFile) {
    ...
    if (config.jobs) {
      ...
      // Set job resolver for job notifiers to enable failure job references
      for (const notifier of Object.values(this.notifiers)) {
        if (notifier.setJobResolver) {
          notifier.setJobResolver(jobName => this.jobs[jobName]);
        }
      }
    }
  }
