...
class Job {
  ...
  constructor(name, {
    ...
    this.onFailureJobName = onFailure;
  ...
  async run(notifyError = true) {
    ...
    try {
      ...
      // Run failure job if configured
      if (this.onFailureJobName && this.getJob) {
        try {
          const failureJob = this.getJob(this.onFailureJobName);
          if (failureJob) {
            log.info(`Running failure job: ${this.onFailureJobName}`);
            await failureJob.run(false); // Don't notify errors from failure jobs to avoid loops
          } else {
            log.warn(`Failure job '${this.onFailureJobName}' not found`);
          }
        } catch (failureErr) {
          log.error('Failure job execution failed', failureErr);
        }
      }
