...
class RunTask {
  ...
  async execute(log) {
    ...
    try {
      ...
      const dockerPromise = this.docker.run(image, this.command, bufferStream.stream, opt);

      let res;
      let dockerContainer;
      if (this.timeout) {
        try {
          [res, dockerContainer] = await Promise.race([dockerPromise, timeoutPromise]);
        } catch (err) {
          // If timeout occurred, try to kill the container
          if (err.message.includes('timed out')) {
            // Get container reference if available
            try {
              const containers = await this.docker.listContainers({
                all: true,
                filters: { ancestor: [image] }
              });
              const recentContainer = containers.find(c => Date.now() - c.Created * 1000 < 30000);
              if (recentContainer) {
                const containerObj = this.docker.getContainer(recentContainer.Id);
                await containerObj.kill().catch(() => {}); // Ignore kill errors
              }
            } catch (killErr) {
              // Ignore errors when trying to kill container
            }
          }
          throw err;
        }
      } else {
        [res, dockerContainer] = await dockerPromise;
      }
